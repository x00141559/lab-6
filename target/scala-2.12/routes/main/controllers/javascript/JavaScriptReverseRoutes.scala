...
package controllers.javascript {
  ...
  class ReverseShoppingCtrl(_prefix: => String) {
    ...
    // @LINE:43
    def shouldCancel: JavaScriptReverseRoute = JavaScriptReverseRoute(
      "controllers.ShoppingCtrl.shouldCancel",
      """
        function() {
          return _wA({method:"GET", url:"""" + _prefix + { _defaultPrefix } + """" + "shouldCancel"})
        }
      """
    )
